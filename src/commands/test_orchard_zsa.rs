...
impl Runnable for TestOrchardZSACmd {
    ...
    fn run(&self) {
        ...
        prepare_test(config.chain.v6_activation_height, &mut wallet, &mut rpc_client);
        ...
        let burn_tx_issuer = create_burn_transaction(issuer, amount_to_burn_issuer as u64, asset, &mut wallet);
        let burn_tx_alice  = create_burn_transaction(issuer, amount_to_burn_alice as u64, asset, &mut wallet);

        mine(&mut wallet, &mut rpc_client, Vec::from([ burn_tx_issuer, burn_tx_alice ]));

        let expected_delta = TestBalances::new(-amount_to_burn_issuer, -amount_to_burn_alice);
        check_balances("=== Balances after burning ===", balances, expected_delta, &mut wallet);
    }
}
