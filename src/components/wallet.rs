...
use zcash_primitives::{
    constants, legacy, sapling::NOTE_COMMITMENT_TREE_DEPTH, transaction::components::Amount,
};
...
impl Wallet {
    ...
    pub(crate) fn orchard_ovk(&self) -> OutgoingViewingKey {
        let sk =
            SpendingKey::from_zip32_seed(self.seed.as_slice(), constants::regtest::COIN_TYPE, 0)
                .unwrap();
...
impl Wallet {
    ...
    pub fn add_notes_from_tx(&mut self, tx: Transaction) -> Result<(), BundleLoadError> {
        ...
        self.add_note_commitments(&tx.txid(), tx.orchard_bundle())
            .unwrap();
    ...
    fn add_notes_from_orchard_bundle(&mut self, txid: &TxId, bundle: &Bundle<Authorized, Amount>) {
        let keys = self
            .key_store
            .viewing_keys
            .keys()
    ...
    pub fn add_note_commitments(
        ...
        let note_commitments: Vec<ExtractedNoteCommitment> =
